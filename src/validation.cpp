...
#if defined(NDEBUG)
# error "Coughingcatcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Coughingcatcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetCoughingcatcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Coughingcatcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("coughingcatcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Coughingcatcoin: BIP30 has been active since inception
    ...
    // Coughingcatcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetCoughingcatcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
{
    // Coughingcatcoin: Disable SegWit
    return false;
    // LOCK(cs_main);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Coughingcatcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Coughingcatcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Coughingcatcoin: We probably want to disable this
    int nLockTimeFlags = 0;

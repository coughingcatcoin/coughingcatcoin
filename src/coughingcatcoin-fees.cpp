...
#ifdef ENABLE_WALLET
//mlumin 5/2021: walletfees, all attached to GetCoughingcatcoinWalletFeeRate which is just the newly exposed ::minWalletTxFee
CAmount GetCoughingcatcoinWalletFee(size_t nBytes_)
{
    //mlumin: super simple fee calc for coughingcatcoin
    CAmount nFee=GetCoughingcatcoinWalletFeeRate().GetFee(nBytes_);
}
...
{
    //mlumin 5/2021: currently 1x COIN or 1 coughingcatcoin or 100,000,000 koinu
...
CAmount GetCoughingcatcoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetCoughingcatcoinDustFee(tx.vout, ::minRelayTxFeeRate);

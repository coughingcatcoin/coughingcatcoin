...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Coughingcatcoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no coughingcatcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("coughingcatcoin"))
        return false;
...
{
    // Convert coughingcatcoin:// to coughingcatcoin:
    //
    //    Cannot handle this later, because coughingcatcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("coughingcatcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "coughingcatcoin:");
    }
...
{
    QString ret = QString("coughingcatcoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Coughingcatcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Coughingcatcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Coughingcatcoin (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Coughingcatcoin\n";
        else
            optionFile << strprintf("Name=Coughingcatcoin (%s)\n", chain);

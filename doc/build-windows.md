...
Below are some notes on how to build Coughingcatcoin Core for Windows.
...
Note that for WSL v1 the Coughingcatcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/coughingcatcoin, AND not under, for example, /mnt/d/coughingcatcoin. 
...
way. This will install to `c:\workspace\coughingcatcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/coughingcatcoin

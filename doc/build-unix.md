# Unix Build of Coughingcatcoin Core
...
You must install required dependencies to build a basic Coughingcatcoin daemon, optional dependencies may vary according to your requirements.
...
Coughingcatcoin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :  
```bash
# Qt (required for coughingcatcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `coughingcatcoind`, `coughingcatcoin-cli` and `coughingcatcoin-qt`.
...
#### Enable Qt GUI
Create `coughingcatcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Coughingcatcoin Core expect BerkeleyDB 5.1**.  
...
#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Coughingcatcoin may be compiled in
...
#### Security
To help make your Coughingcatcoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Coughingcatcoin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Coughingcatcoin Core.
